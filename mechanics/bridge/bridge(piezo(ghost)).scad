...
module cblHole(){
  ...
  rotate([0,0,0])
    for(i=[0:nStrings-1]){
    translate([cblHolePos,i*strDistB-strSpanB/2,0]){
    translate([0,0,0])cube([cblHoleX,cblHoleY,100],center=true);
    }
  }
}
...
module strHole(){
  ...
  rotate([0,0,0])
  for(i=[0:nStrings-1]){
    translate([strHolePos,i*strDistB-strSpanB/2,0]){
    cylinder(d=strHoleDia, h=100,center=true);
    }
  }
}
...
hull(){
translate([0,0,0]){
translate([-1,-x/2,topZ-bockSink])cube([1,x*2,zStrM]);
translate([yA-3-StrMountT,-x/2,topZ-bockSink])cube([3,x*2,yA]);
}
}
...
translate([yA-StrMountT+2,0,topZ-bockSink+strScrwPos])rotate([0,90,0])translate([0,0,-slotXY])cylinder(d=strScrwDia,h=yA,$fn=16);
...
translate([0,yA/2,slotXY/2])rotate([0,0,-90])cblHole();
translate([0,yA/2,slotXY/2])rotate([0,0,-90])strHole();
}
