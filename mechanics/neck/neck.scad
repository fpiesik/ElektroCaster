...
//translate([0,0,-slotXY])frame(slotXY,3); //to make side and board over one thing
...
module btmLck(){
  ...
  for(i=[0:24]){
     if(i==0||i==2||i==4||i==6||i==8||i==11||i==14){
      translate([fretL(fretPos(i))/2-slotXY/2,0,-1.7]){
        translate([0,fretPos(i)+fretSpc(i)/1.75,0])mntSScrw(3.3,10,7,2.5,0);   
      }
      translate([-(fretL(fretPos(i))/2-slotXY/2),0,-1.7]){
        translate([0,fretPos(i)+fretSpc(i)/1.75,0])mntSScrw(3.3,10,7,2.5,0);   
      }
...
union(){
    ...
    rotate([0,0,-0.78])translate([(strSpcN*(nStr-1)+fretStrPlus*2)/2,0,0]){
...
module mntSScrw(dia,z,hDia,hZ,rY){
    ...
    cylinder(d=dia,h=z);
    rotate([0,180,0])cylinder(d=hDia,h=z*2);
    }
}
...
module fretConnect(){
  ...
  pinY=4;
  difference(){
    translate([-x/2,fretPos(1)-endY,0])cube([x,fretPos(nFr)-fretPos(1)+endY*2,z]);
    for(s = [1 : nFr]){
      translate([0,fretPos(s),0])cube([pinX,pinY,z*3],center=true);
    }
  }
}